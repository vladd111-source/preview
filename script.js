...
const __dbg = (msg) => { try {
  ...
  el.style.cssText='position:fixed;bottom:10px;left:10px;max-width:85vw;background:#0b0b0b;color:#00ff7f;padding:6px 8px;font:12px monospace;z-index:99999;border:1px solid #00ff7f;border-radius:6px;opacity:.95';
  el.textContent=String(msg); document.body.appendChild(el); setTimeout(()=>el.remove(),4500);
} catch(_){} };
...
function startGame() {
  ...
  const btn1 = document.createElement('button'); btn1.className='scene-btn'; btn1.innerText='📍 Осмотреться вокруг';
  const btn2 = document.createElement('button'); btn2.className='scene-btn'; btn2.innerText='🚪 Пойти наугад в переулок';
